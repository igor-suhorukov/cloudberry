...
cleanup() {
	for n in $(seq 0 $((NODES - 1))); do
		[ -n "${RESULTS_DIR:-}" ] && cp "$WORK/node$n.log" "$RESULTS_DIR/isolation2-node$n.log" 2> /dev/null
...
# The cluster, as the greenplum suite makes one.  Every node's log is the
# file beside its data directory, which pg_ctl() in setup.sql relies on.
...
# The driver asks database postgres which nodes there are, when a test runs
# a shell command with one's address, and some tests run there: it has
# gp_core too, and so its gp_segment_configuration.
