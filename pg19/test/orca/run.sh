...
# --- identity columns ---------------------------------------------------------
#
# An INSERT that leaves an identity column out, or says DEFAULT for it, and an
# UPDATE that sets one to DEFAULT, take the column's next value from a
# NextValueExpr, which ORCA has no scalar for.  It is not nextval(): it takes
# no USAGE privilege on the sequence.  ORCA is handed a call of a function of
# gp_orca's own for it, and its plan has the NextValueExpr back in the call's
# place (compat/nextvalue.c).

T2I="CREATE TEMP TABLE t2i (id int GENERATED ALWAYS AS IDENTITY, v text)"

dml "an INSERT that leaves an identity column out takes its next value" "Insert on" \
    "INSERT INTO t2i (v) VALUES ('a'); INSERT INTO t2i (v) VALUES ('b')" \
    "SELECT id, v FROM t2i ORDER BY id" "$T2I"

dml "one next value for each row of a multi-row INSERT" "Insert on" \
    "INSERT INTO t2i (v) VALUES ('a'), ('b'), ('c')" \
    "SELECT id, v FROM t2i ORDER BY id" "$T2I"

dml "and for each row that says DEFAULT for it" "Insert on" \
    "INSERT INTO t2i VALUES (DEFAULT, 'x'), (DEFAULT, 'y')" \
    "SELECT id, v FROM t2i ORDER BY id" "$T2I"

dml "and for each row an INSERT takes from a query" "Insert on" \
    "INSERT INTO t2i (v) SELECT 'r' || g FROM generate_series(1, 5) g" \
    "SELECT id, v FROM t2i ORDER BY id" "$T2I"

# Beyond what an integer holds, so a bigint's value is carried as a bigint.
dml "a smallint identity column and a bigint one" "Insert on" \
    "INSERT INTO t2i8 (v) VALUES ('a'), ('b')" \
    "SELECT id, s, v FROM t2i8 ORDER BY id" \
    "CREATE TEMP TABLE t2i8 (id bigint GENERATED ALWAYS AS IDENTITY (START WITH 5000000000),
                             s smallint GENERATED BY DEFAULT AS IDENTITY, v text)"

T2IB="CREATE TEMP TABLE t2ib (id int GENERATED BY DEFAULT AS IDENTITY, v text)"

dml "a value given for one GENERATED BY DEFAULT is kept, and OVERRIDING USER VALUE takes the next" "Insert on" \
    "INSERT INTO t2ib VALUES (100, 'given'); INSERT INTO t2ib OVERRIDING USER VALUE VALUES (200, 'overridden')" \
    "SELECT id, v FROM t2ib ORDER BY id" "$T2IB"

dml "an UPDATE that sets one to DEFAULT" "Update on" \
    "UPDATE t2ib SET id = DEFAULT WHERE v = 'b'" \
    "SELECT id, v FROM t2ib ORDER BY id" \
    "$T2IB; INSERT INTO t2ib VALUES (10, 'a'), (20, 'b')"

got=$(q2 "$T2I" "EXPLAIN (COSTS OFF, VERBOSE) INSERT INTO t2i (v) VALUES ('a')")
case "$got" in
	*identity_nextval*) notok "and the plan shows it as the planner's does" "$got" ;;
	*"nextval('t2i_id_seq')"*"Optimizer: GPORCA"*) ok "and the plan shows it as the planner's does" ;;
	*) notok "and the plan shows it as the planner's does" "$got" ;;
esac

# Whoever may write the table may take its identity column's next value,
# without USAGE on the sequence -- which is why it could not be nextval().
q "DROP ROLE IF EXISTS t2_ins; CREATE ROLE t2_ins;" > /dev/null
T2IR="$T2I; GRANT INSERT, SELECT ON t2i TO t2_ins; SET ROLE t2_ins"

refused "a role with INSERT but no USAGE on the sequence may not call nextval() on it" \
        "$T2IR; SELECT nextval('t2i_id_seq')" "permission denied for sequence t2i_id_seq"

dml "and may still INSERT into the table, taking the column's next value" "Insert on" \
    "INSERT INTO t2i (v) VALUES ('p')" \
    "RESET ROLE; SELECT id, v FROM t2i ORDER BY id" "$T2IR"

# A call of one of those functions that a query makes itself would come back
# from ORCA as the next value too, taken without the privilege nextval()
# asks for.  So ORCA refuses it, the planner leaves it the call it is, and it
# is refused there: nobody but a superuser may execute it, and its body
# raises for a superuser.
#
# From a table: ORCA declines a volatile function in a SELECT with no FROM on
# its own account ("SIRV functions"), which would pass this check without
# reaching the refusal it is about.
T2IC="$T2I; INSERT INTO t2i (v) VALUES ('a')"
got=$(q2 "$T2IC; SET gp.optimizer_trace_fallback = on" \
         "SELECT gp_orca.identity_nextval_int4('t2i_id_seq'::regclass::oid) FROM t2i")
case "$got" in
	*"GPORCA failed to produce a plan"*"a call of gp_orca's function for an identity column's next value"*"cannot be called"*)
		ok "a query that calls the function itself is refused by ORCA, and then by the function" ;;
	*) notok "a query that calls the function itself is refused by ORCA, and then by the function" \
	         "$(printf '%s' "$got" | tr '\n' '|')" ;;
esac

refused "and nobody else may execute it" \
        "$T2IC; GRANT SELECT ON t2i TO t2_ins; SET ROLE t2_ins;
         SELECT gp_orca.identity_nextval_int4('t2i_id_seq'::regclass::oid) FROM t2i" \
        "permission denied for function identity_nextval_int4"

got=$("$PSQL" -X -q -t -A -d postgres -c "$T2IC" \
      -c "SELECT gp_orca.identity_nextval_int4('t2i_id_seq'::regclass::oid) FROM t2i" \
      -c "SELECT last_value FROM t2i_id_seq" 2>&1 | tail -1)
[ "$got" = "1" ] && ok "and the sequence did not move" \
	|| notok "and the sequence did not move" "want [1], got [$got]"

# The functions are gp_orca's extension's, so a database it is not installed
# in has none, and there ORCA leaves such an INSERT to the planner.
q2 "DROP DATABASE IF EXISTS t2_noext" "CREATE DATABASE t2_noext" > /dev/null
got=$("$PSQL" -X -q -t -A -d t2_noext -c "$T2I; SET gp.optimizer_trace_fallback = on" \
      -c "INSERT INTO t2i (v) VALUES ('a')" -c "SELECT id, v FROM t2i" 2>&1)
case "$got" in
	*"GPORCA failed to produce a plan"*"where gp_orca's extension is not installed"*"1|a"*)
		ok "without gp_orca's extension the INSERT goes to the planner, and says why" ;;
	*) notok "without gp_orca's extension the INSERT goes to the planner, and says why" \
	         "$(printf '%s' "$got" | tr '\n' '|')" ;;
esac
q "DROP DATABASE t2_noext;" > /dev/null
