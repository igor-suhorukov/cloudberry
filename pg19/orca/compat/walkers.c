 ...
 *
 * compat/walkers.c
 *	  Walking a plan tree, and the node searches ORCA asks of the server.
 *
 * Cloudberry adds these to src/backend/optimizer/util/walkers.c, a file of
 * its own inside PostgreSQL's tree.  The port does not build that tree, so
 * what ORCA uses of it lives here.
 *
 * Why any of it is needed.  PostgreSQL 19 exports walkers for expressions,
 * queries, range tables and PlanState trees, but none for a *Plan* tree
 * (nodes/nodeFuncs.h).  ORCA produces finished plans without going through
 * the planner, so it has to walk one itself -- to collect the Param ids a
 * plan references, which is what setrefs.c would otherwise have done for it.
 * That is the whole of the framework's use here.
 *
 * What was dropped, and why it is safe.  Cloudberry's walker has a case for
 * each of its MPP plan nodes -- Motion, ShareInputScan, the six Dynamic
 * scans, AssertOp, Sequence, SplitUpdate, SplitMerge, TupleSplit,
 * PartitionSelector, RuntimeFilter, WindowHashAgg -- and for its expression
 * nodes DMLActionExpr, DQAExpr and TableValueExpr, none of which exist on
 * PostgreSQL 19.  They are left out rather than stubbed: at this milestone
 * ORCA plans for one segment and emits none of them, and a case for a node
 * that cannot arrive is a case nobody can test.  They come back with the
 * nodes themselves, at M2.
 *
 * Plan.flow goes the same way, being a field Cloudberry adds to Plan.
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * Cloudberry has plan_tree_base_subplan_put_plan() here, the setter its
 * mutators use.  It is not here: it needs exec_subplan_put_plan() and
 * planner_subplan_put_plan(), which Cloudberry adds to PostgreSQL and
 * PostgreSQL 19 does not have, and ORCA only ever walks a plan tree -- it
 * never rewrites one through this framework.
 */
...
extract_nodes_walker(Node *node, extract_context *context)
{
	...
	/*
	 * Cloudberry stops here for a TableFunctionScan as well.  That node does
	 * not exist on PostgreSQL 19, so there is nothing to stop at; it comes
	 * back with the node.
	 */
