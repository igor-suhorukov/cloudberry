...
// under the License.
//
//	Ported from github/cloudberry/src/backend/gpopt/utils/COptTasks.cpp,
...
GenerateJoinNodes(CMemoryPool *mp, List *relations)
{
	...
	ListCell *l;
	int count = 0;
	// ForEachWithCount, the wrapper layer's spelling of the same loop:
	// foreach_with_count is Cloudberry's addition to pg_list.h.
	ForEachWithCount(l, relations, count)
	{
...
COptTasks::OptimizeTask(void *ptr)
{
	...
	GPOS_TRY
	{
...

		{
			...
			// the session as well where there is one node, and a count of
			// the queries this backend has sent to ORCA names the command.
			static ULONG optimized_query_count = 0;
			...
			plan_dxl = COptimizer::PdxlnOptimize(
				...
				(ULONG) MyProcPid, ++optimized_query_count,
