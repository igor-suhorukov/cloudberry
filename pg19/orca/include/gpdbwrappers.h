...
//
//	@doc:
//		Everything the translator is allowed to ask the server.
//
//		Ported from github/cloudberry/src/include/gpopt/gpdbwrappers.h.  The
//		declarations are the contract the 12 translator files are written
//		against, so they are carried whole, including the ones the port
//		cannot answer yet: a wrapper for a feature that belongs to a later
//		milestone is declared here and raises when called, so that ORCA falls
//		back rather than plans something wrong.  gpdbwrappers.cpp says which
//		ones, and names the milestone for each.
//
//		WHO CALLS THIS.  Not ORCA.  Its four core libraries reference exactly
//		one of the 198 wrappers -- gpdb::IsParallelModeOK, from two files in
//		libgpopt/src/xforms -- and nothing else here.  Everything else is
//		called from the translator, from the relcache translator and from
//		gpopt/utils, which is to say from the port's own code.  The plan's
//		"gpdbwrappers.cpp next, because everything calls it" is right about
//		the translator and wrong about ORCA; see "Corrections to the plan" in
//		cloudberry.md.
//
//		WHY IT EXISTS.  PostgreSQL reports errors by longjmp and ORCA by C++
//		exception, and neither unwinds the other's stack.  Every function
//		here is a place where one becomes the other: GP_WRAP_START in
//		gpdbwrappers.cpp installs a jump target, and a PostgreSQL error
//		arriving there is re-raised as gpdxl::ExmiGPDBError.  Calling a
//		server function from the translator without coming through this file
//		leaks ORCA's memory pools at best, and crashes at worst.
//
//---------------------------------------------------------------------------
...
/*
 * The compat layer, for what Cloudberry added to the files above.  CmpType
 * comes from cb_lsyscache.h and several declarations below use it;
 * utils/faultinjector.h is the port's own, written over PostgreSQL 19's
 * injection points, and is where FaultInjectorType_e comes from.
 */
...
// Int8FromDatum and DatumFromInt8 are not here.  PostgreSQL 19 has no
// DatumGetInt8 or Int8GetDatum -- int8 is a signed char and the Char
// accessors are the ones it keeps -- and nothing in the translator called
// either wrapper.  They are left out rather than re-spelled over
// DatumGetChar, so that a caller that appears later has to say which of the
// two it means.
...
//
// MemCtxtAllocZeroAligned is not among them: PostgreSQL 19 removed
// MemoryContextAllocZeroAligned, and its only caller was Palloc0Fast at the
// foot of this header, which went in the same upstream commit.
...
// (always compiled: without a build configured for injection points
// the macros behind it compile to nothing, which is the right answer)
...
// `const IndexAmRoutine *` and the translator only reads it.
const IndexAmRoutine *GetIndexAmRoutineFromAmHandler(Oid am_handler);
...
/*
 * Allocating a node through the wrapper layer, so that an out-of-memory
 * error becomes a GPOS exception rather than a longjmp past C++ frames.
 *
 * Cloudberry has this as a pair of macros with a __GNUC__ split, because
 * PostgreSQL 16's newNode() had to be an expression and so needed either a
 * statement-expression or a global holder variable in order to assign the
 * tag.  PostgreSQL 19 made newNode() a static inline function (3c080fb4fad,
 * "Simplify newNode() by removing special cases"), and this follows it: one
 * function, no split, no holder.
 *
 * Palloc0Fast went with it.  It chose between MemCtxtAllocZeroAligned and
 * MemCtxtAllocZero on MemSetTest(), and PostgreSQL 19 has none of the three.
 */
namespace gpdb
{
static inline Node *
NewNode(Size size, NodeTag tag)
{
	...
	result = (Node *) MemCtxtAllocZero(CurrentMemoryContext, size);
	result->type = tag;
...
#define MakeNode(_type_) ((_type_ *) gpdb::NewNode(sizeof(_type_), T_##_type_))
