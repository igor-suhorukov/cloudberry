	...
	const Expr *expr, const CMappingVarColId *var_colid_mapping)
{
	...
	// still refuses DISTINCT in a window function ("DISTINCT is not
	// implemented for window functions", parse_func.c), so a WindowFunc
	// never asks for it.
	CDXLScalarWindowRef *winref_dxlop = GPOS_NEW(m_mp) CDXLScalarWindowRef(
		...
		false /* is_distinct */, window_func->winstar, window_func->winagg,
