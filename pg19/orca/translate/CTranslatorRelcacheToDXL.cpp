...
// Not cdb/cdbhash.h: Cloudberry includes it here for HASH_AM_OID alone,
// which PostgreSQL 19 defines in catalog/pg_am_d.h, through pg_am.h above.
#include "partitioning/partdesc.h"
#include "utils/array.h"
#include "utils/datum.h"
#include "utils/elog.h"
#include "utils/fmgroids.h"	 // F_COUNT_ANY, which Cloudberry reached another way
#include "utils/guc.h"

// Cloudberry declares every optimizer_* setting in its patched
// utils/guc.h; PostgreSQL 19's declares none of them.
									  ...
									  IMDId *mdid)
{
	...
	// The policy is read once, here, rather than off the relcache entry:
	// rd_cdbpolicy is Cloudberry's field, and the port keeps the policy in
	// the "gp" label instead.
	//
	// On one node every relation is reported as coordinator-only, whatever
	// its label records, because every row is here -- and that is what keeps
	// ORCA from planning a Motion.  COptTasks turns Motions off for a query
	// that touches no distributed table.  A table with a DISTRIBUTED BY label,
	// which gp_sql writes at M1 because M2 needs it and pg_dump carries it,
	// would otherwise count as one, and be given a Gather Motion from a
	// segment that does not exist.  Cloudberry's own single-node mode gets
	// the same effect by discarding the clause in its grammar.  A null
	// policy is what makes both branches below answer EreldistrMasterOnly.
	const BOOL single_node = gpdb::IsSingleNode();
	GpPolicy *gp_policy =
		single_node ? nullptr : gpdb::GetDistributionPolicy(rel.get());

	if (nullptr != gp_policy && POLICYTYPE_ENTRY != gp_policy->ptype &&
		gpdb::GetGPSegmentCount() != gp_policy->numsegments)
	{
	...
	// the distribution policy was read above
	...
	// On one node no partition is distributed, so none can be distributed
	// differently from its parent.
	convert_hash_to_random =
		!single_node && gpdb::IsChildPartDistributionMismatched(rel.get());
											 ...
											 Relation rel)
{
	...
	{
		// TupleDescAttr() rather than &rd_att->attrs[ul]: PostgreSQL 18 gave
		// TupleDesc an array of CompactAttributes and moved the full
		// pg_attribute rows behind it, so there is no attrs member to index.
										...
										IMDId *mdid_index)
{
	...
	switch (index_rel->rd_rel->relam)
	{
		...
		case GIST_AM_OID:
			...
			break;
		default:
			// The access methods that are Cloudberry's own have no fixed OID in the
			// port -- each is created by its module's CREATE EXTENSION -- so they are
			// found by name, in the default branch; see gpdb::IsAccessMethodNamed.
			if (gpdb::IsAccessMethodNamed(index_rel->rd_rel->relam, "bitmap"))
			{
	...
	// const: PostgreSQL 19's GetIndexAmRoutine returns one, and so does the
	// wrapper over it.
	const IndexAmRoutine *am_routine =
										   ...
										   IMDId *mdid)
{
	...
	// No NDV-by-segments slot.  It is Cloudberry's statistic kind 8, written
	// only by Cloudberry's own ANALYZE; the port's ANALYZE is PostgreSQL 19's
	// do_analyze_rel (O3), which never writes it, so the lookup would always
	// miss.  Worse, PostgreSQL 19 numbers its own kinds 1 to 7, and a lookup
	// by Cloudberry's number would read whatever a later PostgreSQL puts at 8.
	// The default is what Cloudberry uses when the slot is absent.
...
CTranslatorRelcacheToDXL::RetrieveRelStorageType(Relation rel)
{
	...
	switch (rel->rd_rel->relam)
	{
		...
		default:
		{
			...
			if (gpdb::IsAccessMethodNamed(relam, "pax"))
			{
				rel_storage_type = IMDRelation::ErelstoragePAX;
				break;
			}
			if (gpdb::IsAccessMethodNamed(relam, "ao_column"))
			{
				rel_storage_type = IMDRelation::ErelstorageAppendOnlyCols;
				break;
			}
			if (gpdb::IsAccessMethodNamed(relam, "ao_row"))
			{
...
CTranslatorRelcacheToDXL::IsIndexSupported(Relation index_rel)
{
	...
	BOOL index_supported = gpdb::HeapAttIsNull(tup, Anum_pg_index_indexprs) &&
						   ...
						   (BTREE_AM_OID == index_rel->rd_rel->relam ||
							...
							gpdb::IsAccessMethodNamed(index_rel->rd_rel->relam,
													  "bitmap") ||
...
{
	// No RELKIND_AOSEGMENTS: it is Cloudberry's relkind 'o', for an AO
	// table's segment-file table, and the port's AO tables keep theirs as
	// ordinary tables, which RELKIND_RELATION already answers for.
