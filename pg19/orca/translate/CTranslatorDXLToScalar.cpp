...
// under the License.
//
//	Ported from github/cloudberry/src/backend/gpopt/translate/CTranslatorDXLToScalar.cpp,
	...
	const CDXLNode *scalar_winref_node, CMappingColIdVar *colid_var)
{
	...
	// Not window_func->windistinct, which is Cloudberry's field: PostgreSQL
	// 19 has no DISTINCT in a window function, and the query side of the
	// translator always says so (CTranslatorScalarToDXL).  ORCA adds none.
	GPOS_ASSERT(!dxlop->IsDistinct());
	...
	const CDXLNode *scalar_subplan_node, CMappingColIdVar *colid_var)
{
	...
	// PostgreSQL 19 has no SubLinkType for NOT EXISTS: the parser writes it
	// as NOT over an EXISTS sublink, and NOT_EXISTS_SUBLINK is Cloudberry's.
	// So a NOT EXISTS subplan is built as the EXISTS subplan, and the NOT
	// goes over it on the way out.  EXISTS is never NULL, so NOT of it is
	// exactly what Cloudberry's executor computes for NOT_EXISTS_SUBLINK.
	const BOOL is_not_exists =
		(EdxlSubPlanTypeNotExists == dxlop->GetDxlSubplanType());
	SubLinkType slink = is_not_exists
							? EXISTS_SUBLINK
	...
	// A subplan with no parameters that is not hashed is rescanned from the
	// start for every evaluation, and PostgreSQL's executor keeps its rows
	// for that only when told to, through PlannedStmt.rewindPlanIDs -- which
	// build_subplan() fills, and which is what makes a Material at the top of
	// the subplan keep its tuplestore (see TranslateDXLMaterialize).  An
	// InitPlan would be excluded too, but ORCA makes none.
	if (subplan->parParam == NIL && !subplan->useHashTable)
	{
		dxl_to_plstmt_ctxt->AddRewindPlanId(subplan->plan_id);
	}

	if (is_not_exists)
	{
		BoolExpr *not_expr = MakeNode(BoolExpr);
		not_expr->boolop = NOT_EXPR;
		not_expr->args = gpdb::LAppend(NIL, subplan);
		not_expr->location = -1;

		return (Expr *) not_expr;
	}
...
{
	// NOT EXISTS reaches here as EXISTS_SUBLINK; see
	...
	Plan *plan, SubLinkType slink, CContextDXLToPlStmt *dxl_to_plstmt_ctxt)
{
	...
	// PostgreSQL 19's name for Cloudberry's is_initplan; ORCA makes no
	// InitPlans, so it is false either way.
	subplan->isInitPlan = false;
	...
	// Not is_multirow, which is Cloudberry's field and always false here.
