...
CTranslatorUtils::ConvertToDXLJoinType(JoinType jt)
{
	...
	switch (jt)
	{
		...
		// No JOIN_LASJ_NOTIN: it is Cloudberry's JoinType, for the NOT IN
		// anti-join its own planner builds.  PostgreSQL 19 has no such
		// value, and a Query's JoinExpr carries none of the planner's join
		// types in any case.
...
CTranslatorUtils::MapDXLSubplanToSublinkType(EdxlSubPlanType dxl_subplan_type)
{
	...
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
				   GPOS_WSZ_LIT("NOT EXISTS as a sublink type"));
	}
...
{
	// No NOT_EXISTS_SUBLINK: PostgreSQL 19's parser writes NOT EXISTS as
	// NOT over an EXISTS sublink, and has no value for it to be.
