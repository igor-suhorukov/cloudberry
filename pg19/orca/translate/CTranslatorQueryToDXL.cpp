...
// under the License.
//
//	Ported from github/cloudberry/src/backend/gpopt/translate/CTranslatorQueryToDXL.cpp,
...
#include "CQueryMutators.h"
#include "CTranslatorDXLToPlStmt.h"
#include "CTranslatorQueryToDXL.h"
	  ...
	  m_cteid_at_current_query_level_map(nullptr)
{
	...
	// Row locks: FOR UPDATE, FOR SHARE and the rest.  Cloudberry's translator
	// reads no rowMarks, because Cloudberry answers a locking clause with a
	// lock on the whole table, ExclusiveLock, taken when the statement is
	// analysed (planner.c, "the conflict with UPDATE|DELETE is implemented by
	// locking the entire table").  PostgreSQL 19 locks the rows, in the
	// LockRows node its plan has to have, and does nothing else -- so a plan
	// built here would lock nothing at all.
	if (NIL != query->rowMarks)
	{
...
CTranslatorQueryToDXL::CheckUnsupportedNodeTypes(Query *query)
{
	static const SUnsupportedFeature unsupported_features[] = {
		...
		// Not T_GroupId: GROUP_ID() is Cloudberry's, and PostgreSQL 19 has
		// no node for it to refuse.
...
CTranslatorQueryToDXL::CheckSupportedCmdType(Query *query)
{
	...
	{
		// Cloudberry refuses three kinds of SELECT here, which its Query
		// marks with parentStmtType: the body of a CREATE TABLE AS while
		// gp.optimizer_enable_ctas is off, of a COPY to a file on the
		// segments, and of a REFRESH MATERIALIZED VIEW.  PostgreSQL 19's
		// Query has no such field and needs none.  All three hand the planner
		// a plain SELECT and put its rows where they go through a
		// DestReceiver, so on one node the plan is a plain SELECT's.  What
		// Cloudberry's CTAS adds to it, a distribution for the new table, is
		// M2's -- and PostgreSQL 19 does not tell the planner it is planning
		// one, so M2 has to find that out another way.
		return;
	}
...
CTranslatorQueryToDXL::TranslateQueryToDXL()
{
	...
	{
		case CMD_SELECT:
			// Always a SELECT: see CheckSupportedCmdType for why a CTAS is
			// one too.  TranslateCTASToDXL is M2's.
...
CTranslatorQueryToDXL::TranslateFromClauseToDXL(Node *node)
{
	...
	if (IsA(node, RangeTblRef))
	{
		...
		// Not rte->forceDistRandom: gp_dist_random(), which reads a catalog
		// table on every segment, is Cloudberry's, and PostgreSQL 19's range
		// table entry has no field for it.
...
CTranslatorQueryToDXL::UnsupportedRTEKind(RTEKind rtekind)
{
	...
	switch (rtekind)
	{
		...
		// Not RTE_VOID or RTE_TABLEFUNCTION, which are Cloudberry's kinds.
		// These three are PostgreSQL's, and newer than the translator: a
		// transition table read in an AFTER trigger, an empty FROM clause the
		// planner makes (never in a query it is handed, but named so that the
		// switch covers the enum), and PostgreSQL 18's grouping step, which
		// optimize_query() has already folded back into the query above it.
...
CTranslatorQueryToDXL::NoteDistributionPolicyOpclasses(const RangeTblEntry *rte)
{
	...
	if (rte->rtekind == RTE_RELATION)
	{
		...
		// The policy the relcache translator reports, read the way it reads
		// it: from the "gp" label rather than Cloudberry's rd_cdbpolicy, and
		// not at all on one node, where every relation is coordinator-only
		// and so has no distribution key to hash (see
		// CTranslatorRelcacheToDXL::RetrieveRel).
		GpPolicy *policy = gpdb::IsSingleNode()
							   ? nullptr
							   : gpdb::GetDistributionPolicy(rel.get());
		...
		for (int i = 0; i < policy_nattrs; i++)
		{
			...
			// TupleDescAttr() rather than desc->attrs[], which PostgreSQL 19's
			// TupleDesc no longer has as an array of attributes.
